...
env = gym.make('OneAgentFlying-v0')
pdb.set_trace()
if not os.path.exists('./logs'):
	os.makedirs('./logs')

filename = str(datetime.datetime.now())+str('_%dagents_fcnpolicy'%env.n_agents)
...
def main(episodes):
	...
	for episode in range(episodes):
		...
		g = 0 #this is ok no graph is used cuz fcn.
		done = False
	...
	#pdb.set_trace()
	np.savetxt('v2_Relative_Goal_Reaching_for_%d_agents_rs_rg.txt' %(env.n_agents), plotting_rew)
	fig = plt.figure()
	x = np.linspace(0,len(plotting_rew),len(plotting_rew))
	plt.plot(x,plotting_rew)
	plt.savefig('v2_Relative_Goal_Reaching_for_%d_agents_rs_rg.png' %(env.n_agents))
	plt.show()
